...
const store = {
  ...
  getItemPrice(itemName) {
    const checkPrice = this.isItemInStore(itemName);
    let itemPrice;
    if (checkPrice === false) {
      itemPrice = -1;
  ...
  getItemQuantity(itemName) {
    ...
    if (checkStore === false) {
      ...
      const findItem = inventory.find((item) => item.name === itemName);
      itemQuantity = findItem.quantity;
    }
    return itemQuantity;
  },
  ...
  addItemQuantity(itemName, price, quantity) {
    ...
    if (checkStore === true) {
      ...
      const item = { name: itemName, price: itemPrice, quantity: Quant };
      inventory.push(item);
      updatedQuant = item.quantity;
    }
    return updatedQuant;
  },
  /**
